...
<div class="main">
				...
				<div class="col-md-12">
					<div class="panel">
								<div class="panel-heading">
									...
									<h3 class="panel-title">Profil KK Rekayasa Perangkat Lunak dan Multimedia</h3>
									<div class="right">
									<!-- <a href="/pkrd/export" class="btn btn-primary">Export Excel</a>
									<a href="/pkrd/exportPdf" class="btn btn-primary">Export PDF</a> -->
										@role('rplm')
									<button type="button"  data-toggle="modal" data-target="#tambahRplm" style="background-color:#00AAFF; color: white;padding: 6px 22px; box-shadow: 0px 1px 2px 0 rgba(0, 0, 0, 0.2);">Create</button>
									@endrole
									</div>
								</div>
								<div class="panel-body">
									<table class="table table-hover" id="datatable">
										<thead>
                                         ...
                                         <tbody>
                                         	@foreach($data_rplm as $rplm)
                                         	<tr>
                                         		<td>{{$rplm->namadosen}}</td>
                                         		<td>{{$rplm->nidn}}</td>
                                         		<td>{{$rplm->prodi}}</td>
                                         		@php
                                         		$coba=explode(',',$rplm->id_topic)
                                         		@endphp
                                         			<td>
                                         		 <ul>
                                         		...
                                         		<td>
                                         			...
                                         			<a href="/rplm/{{$rplm->id}}/edit" class="btn btn-warning btn-sm">Edit</a>
                                         			<a href="/rplm/{{$rplm->id}}/delete" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Ingin Menghapus?')">Delete</a>
	...
	<div class="modal fade" id="tambahRplm" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      <div class="modal-header">
			        <h5 class=" modal-title" id="exampleModalLabel">Masukan profil dosen</h5>
			        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
			          <span aria-hidden="true">&times;</span>
			        </button>
			      </div>
			      <div class="modal-body">
			      	<form action="{{route('createRplmTes1')}}" method = "POST">
			      		{{csrf_field()}}
			        <form>
