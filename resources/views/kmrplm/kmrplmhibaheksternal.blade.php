				...
				<div class="col-md-12">
					<div class="panel">
								<div class="panel-heading">
									...
									<h3 class="panel-title">Kontrak Manajemen Hibah Eksternal KK Rekayasa perangkat Lunak dan Multimedia</h3>
									<div class="">
									<button type="button"  data-toggle="modal" data-target="#exampleModal" style="background-color:#e4cb10; color: white;padding: 6px 22px; box-shadow: 0px 1px 2px 0 rgba(0, 0, 0, 0.2);">Create</button>
									 <div class="dropdown show">
											  ...
											  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
											  	<ul>
											  		<li><a href="/kmrplm/kmrplmhibaheksternal/export">EXPORT SEMUA</a></li>
											  		<li><a class="dropdown-item" href="/kmrplm/kmrplmhibaheksternal/export/TW1">TW 1</a></li>
											  		<li><a class="dropdown-item" href="/kmrplm/kmrplmhibaheksternal/export/TW2">TW 2</a></li>
											  		<li><a class="dropdown-item" href="/kmrplm/kmrplmhibaheksternal/export/TW3">TW 3</a></li>
											  		<li><a class="dropdown-item" href="/kmrplm/kmrplmhibaheksternal/export/TW4">TW 4</a></li>
								...
								<div class="panel-body">
									...
									<table class="table table-hover" id="datatable">
										<thead>
                                         ...
                                         @foreach($data_kmrplmhibaheksternal as $kmrplmhibaheksternal)
                                         	<tr>
                                         		<td>{{$kmrplmhibaheksternal->judulpenelitian}}</td>
                                         		<td>{{$kmrplmhibaheksternal->periode}}</td>
                                         		<td>{{$kmrplmhibaheksternal->peneliti1}}</td>
                                         		<td>{{$kmrplmhibaheksternal->anggotapeneliti2}}</td>
                                         		<td>{{$kmrplmhibaheksternal->anggotapeneliti3}}</td>
                                         		<td>{{$kmrplmhibaheksternal->anggotapeneliti4}}</td>
                                         		<!-- <td>{{$kmrplmhibaheksternal->kelompokkeahlian}}</td> -->
                                         		<td>Rp {{number_format($kmrplmhibaheksternal->dana_diajukan,0,'.','.')}}</td>
                                         		<td>Rp {{number_format($kmrplmhibaheksternal->dana_disetujui,0,'.','.')}}</td>
                                         		<td>{{$kmrplmhibaheksternal->kategori}}</td>
                                         		<td>{{$kmrplmhibaheksternal->keterangan}}</td>
                                         		<td>{{$kmrplmhibaheksternal->status}}</td>
                                         		@role('rplm')
                                         		<td>
                                         			<a href="/kmrplm/kmrplmhibaheksternal/{{$kmrplmhibaheksternal->id}}/edithe" class="btn btn-warning btn-sm">Edit</a>
                                         			<a href="/kmrplm/kmrplmhibaheksternal/{{$kmrplmhibaheksternal->id}}/delete" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Ingin Menghapus?')">Delete</a>
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			      	<form action="/kmrplm/kmrplmhibaheksternal/create" method = "POST" >
			      		{{csrf_field()}}
			        <form>
