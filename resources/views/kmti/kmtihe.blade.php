				...
				<div class="col-md-12">
					<div class="panel">
								<div class="panel-heading">
									...
									<h3 class="panel-title">Kontrak Manajemen Hibah Eksternal KK Teknologi Informasi</h3>
									<div class="">
									<button type="button"  data-toggle="modal" data-target="#exampleModal" style="background-color:#e4cb10; color: white;padding: 6px 22px; box-shadow: 0px 1px 2px 0 rgba(0, 0, 0, 0.2);">Create</button>
									 <div class="dropdown show">
											  ...
											  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
											  	<ul>
											  		<li><a href="/kmti/kmtihe/export">EXPORT SEMUA</a></li>
											  		<li><a class="dropdown-item" href="/kmti/kmtihe/export/TW1">TW 1</a></li>
											  		<li><a class="dropdown-item" href="/kmti/kmtihe/export/TW2">TW 2</a></li>
											  		<li><a class="dropdown-item" href="/kmti/kmtihe/export/TW3">TW 3</a></li>
											  		<li><a class="dropdown-item" href="/kmti/kmtihe/export/TW4">TW 4</a></li>
								...
								<div class="panel-body">
									...
									<table class="table table-hover" id="datatable">
										<thead>
                                         ...
                                         @foreach($data_kmtihe as $kmtihe)
                                         	<tr>
                                         		<td>{{$kmtihe->judulpenelitian}}</td>
                                         		<td>{{$kmtihe->periode}}</td>
                                         		<td>{{$kmtihe->peneliti1}}</td>
                                         		<td>{{$kmtihe->anggotapeneliti2}}</td>
                                         		<td>{{$kmtihe->anggotapeneliti3}}</td>
                                         		<td>{{$kmtihe->anggotapeneliti4}}</td>
                                         		<!-- <td>{{$kmtihe->kelompokkeahlian}}</td> -->
                                         		<td>Rp {{number_format($kmtihe->dana_diajukan,0,'.','.')}}</td>
                                         		<td>Rp {{number_format($kmtihe->dana_disetujui,0,'.','.')}}</td>
                                         		<td>{{$kmtihe->kategori}}</td>
                                         		<td>{{$kmtihe->keterangan}}</td>
                                         		<td>{{$kmtihe->status}}</td>
                                         		@role('ti')
                                         		<td>
                                         			<a href="/kmti/kmtihe/{{$kmtihe->id}}/edithe" class="btn btn-warning btn-sm">Edit</a>
                                         			<a href="/kmti/kmtihe/{{$kmtihe->id}}/delete" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Ingin Menghapus?')">Delete</a>
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			      	<form action="/kmti/kmtihe/create" method = "POST" >
			      		{{csrf_field()}}
			        <form>
